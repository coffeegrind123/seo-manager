...
| Script | Job |
|---|---|
| `seostate.py` | **all state**: queue, keywords, ranks, pages, trends, prospects, profile, conventions, pacing, overview, next-actions, run log — and since 2026-09-14 the **deferred list** (`defer` / `deferred --due` / `deferred-update`): every candidate a gate withheld, with a class (`authority` / `catalogue` / `brand_navigational` / `dev_intent` / `off_remit`) and a revisit condition. `catalogue` files the page-1 aggregators as prospects; `brand_navigational` folds the keyword into a crediting page; `authority` re-opens at the next DR band and `next-actions` says when. `propose` refuses a keyword that is still deferred |
| `serp.py` | live SERPs through the provider ladder, plus the weakness/authority scoring the gate needs. **Failover is the default** (serpd → serper → serpapi → ddg; `--no-fallback` pins one) and `fell_back_from` names the provider that failed. `unresolved_wrappers` lists results whose host could not be recovered from Google's `/goto?url=` redirect — counted nowhere, never as authority |
...
| `bing.py` | **the only free source of real search volume AND backlinks** — Bing Webmaster Tools for a site you own. Impressions per query, related-keyword expansion with numbers, inbound links, query stats, `urlinfo` for Bing's own per-URL index record (discovered / last crawled / size — its answer to Google's URL Inspection), and `pages`/`pagequeries` for the PAGE dimension. **Also the CRAWLER's side of the same account** — `crawlstats` (fetched per day, code mix, pages in index), `crawlissues` (per-URL, empty answer guarded against `crawlstats`), `feeds --verify` (the sitemap as Bing holds it vs the live file), `blocked` (account-level blocks, invisible to any crawl), `crawlsettings` (a self-imposed rate cap), `quota` + `submit` (the one crawl-acceleration lever here, dry-run until `--yes`, and NOT IndexNow's channel or quota) — which URL actually earns the clicks, and for which searches. `queries` alone cannot tell a locale page ranking in its own language from the homepage ranking for that language, and those two have opposite fixes |
...
| `serpd.py` | **the fast path for SERP-heavy runs**: a long-lived headed Chrome behind `localhost:8791`. One curl per check, real Google, no DOM in your context. 25 checks in ~37s and 1.4KB of verdicts. `/batch` spools every result to `/tmp/seo-serpd-batches/<id>.json` before replying and `GET /batch?id=` returns it, so a client timeout no longer loses the batch — pass your own `"id"`. `/health` reports `throttled` when Google served `/sorry` in the last 15 min |
| `indexnow.py` | get published pages crawled: IndexNow ping (free, keyless, Bing/Yandex/Seznam/Naver) + the batched Google "Request indexing" follow-up. Google's automatable half is `gsc.py sitemap-submit`; only per-URL requests are still a human clicking a button |
| `gsc.py` | **Google Search Console, in this skill** — `sites` (also the auth control), `sitemaps` (when Google last DOWNLOADED each feed, which is the field that matters — `lastSubmitted` only records that you asked), `sitemap-submit` (the ONE programmatic nudge Google offers, and it asks for a feed re-read, not for a URL to be indexed; dry run until `--yes`), `inspect` (URL Inspection — `canonical_agrees: false` is the finding to act on), `query` (search-analytics in Google's OWN row shape, so `decay.py`/`keywords.py gsc` consume it unchanged), `decay-export` (both adjacent equal-length windows `decay.py compare` wants, in one command). RS256 signed in pure Python — no `openssl`, no wheel. ⚠ Three guarded traps: `contents[].indexed` is a stuck counter reading 0 on properties with thousands of indexed URLs (returned as `null`), every count arrives as a **string** (`"9" > "5388"`), and the data lags 2-3 days so a fully-lagged window is REFUSED rather than answered with zero rows |
| `postdeploy.py` | **the whole post-deploy sequence, behind a health gate**: `/`, `/sitemap.xml` and `/robots.txt` must all be 200 before anything is announced (a release swap can 404 a site for minutes, and a CDN caches those 404s for hours), then `contract.py check`, then IndexNow, then `gsc.py sitemap-submit`. Submissions are a DRY RUN until `--yes`; the checks always run. Every run appends to `.seo/postdeploy.jsonl`, because IndexNow returns 200 and keeps no queryable history — "did we announce that deploy?" was genuinely unanswerable once, and an action with no receipt cannot be audited |
...
| `seodoctor.py` | **self-healing preflight** - idempotent check+repair of the X display (a dead Xvfb leaves a stale socket that presents as "chrome did not bind CDP"; the doctor restarts it), the daemon (a Google-throttled one is restarted on a fresh proxy session), its Chrome, deps and project state. Run it first, every run. `--providers` adds a live sweep of every data source |
...
| `crawllog.py` | **the access log**: crawl budget by silo, status codes served to bots, AI-crawler ingestion, and reverse+forward DNS bot verification. `--remote` runs the aggregation on the server so the log never crosses the wire. `urls --keep-query` when a single path's hit count looks like a budget finding - by default the query is stripped, so 408 parameterised URLs and one page fetched 506 times are the same row. **`--bot` makes the spoof detector blind and now says so (`hits_net: null`) instead of reporting a false clean.** **Every row is netted against the UA-spoofing detector — read `hits_net`, never `hits`**, and treat anything in `bots_entirely_forged` as "did not visit" rather than as a small number; measured 2026-09-01 that was 65% of `ai_search`, 62% of `ai_user` and 92% of `social`, including every Anthropic row |
...
| `remeasure.py` | **hypotheses with baselines, re-checked by re-running the SAME command**. A change ships and the honest question is 'did it work'. Four ways that gets answered wrongly, and this refuses each: re-measuring with a DIFFERENT command (the argv is stored with the baseline and a mismatch is refused, not compared - this skill has already produced a fake '100% page-1 churn' from two extraction regimes); deciding the expected direction afterwards (direction and threshold are pre-registered); checking too early (`not_before`, because a crawl hypothesis at day three reads noise as an answer); and reading 'cannot measure' as 'no change' (a throttled provider and a flat metric produce the same empty screen — the first is `unmeasured` and leaves the hypothesis open). `record` REFUSES a duplicate - an experiment's identity is what it runs and what it reads, not the label somebody gave it, and two baselines for one intervention can disagree with nothing to say which to believe (this fired on its author within the hour). `close --id X --reason ...` retires one; the reason is required, because closing without one is indistinguishable from closing because the answer was inconvenient. `--metric` addresses a list element by **identity** (`bots.bingbot.hits`), not position: `bots.0` into a value-sorted list silently re-points itself between runs, so the verdict would compare two different bots and call it a change |
...
| `geo.py` | **do answer engines CITE us, and is what they say right?** `crawllog` measures who CRAWLED and `backlinks referrers` who REFERRED; neither asks whether an assistant answering a question this site exists to answer names this site. The unit is an ANSWER, not a ranking - a page can sit at position 3 and be absent from the overview above it. Google's AI Overview is reachable today through the SERP key; the LLM engines are wired and report `no_key` until one exists. `extractable` is the keyless half: an assistant lifts a SENTENCE, so a page can rank and still never be the sentence quoted |
...
| `test_bing.py` | the controls for the Bing page dimension AND the crawler side: a running total summed over days (`Code2xx` = 96,000 for 7,408 pages crawled), a reference/measurement disagreement on a column's kind that must SHOW rather than be overridden, an empty `crawlissues` that must read `unknown` when `crawlstats` logged errors, the SECOND never-sentinel (`/Date(-11644473600000)/` = the Windows FILETIME epoch, which decodes to a well-formed `1601-01-01` and shipped that way), and `submit`'s guards firing BEFORE the network — off-site dropped, over-quota refused whole, nothing sent on a dry run. Also the original page-dimension set: `GetPageStats` returning the page URL in a field literally named `Query`, per-date rows that only total after aggregation, click-sorting vs impression-sorting (which invert on real data), CTR on a zero-impression page, and an empty result that must name BOTH "no data yet" and "you typed the URL wrong" rather than implying the page is dead. Also the `urlinfo` decoder — .NET `DateTime.MinValue` (`/Date(-62135568000000-0800)/`) is "never", not year 0001 — and the refusal of `--days` on the four subcommands whose endpoints have no date range, with controls that `keyword`/`expand` still accept it |
...
| `test_geo.py` | the controls for answer sampling: that an unaskable engine REFUSES rather than reporting `not_cited`, that a SERP with no overview is `no_answer_surface` and excluded from the rate rather than counted as a miss, that share of voice is one vote per answer (it read 4.2 on real data), and that the engine's own plumbing is excluded but visibly so |
| `test_remeasure.py` | the controls for the runner: that a missing metric is `unmeasured` and never a zero (the failure reports a collapse that never happened), that a changed command is refused rather than compared, that checking before the window refuses, and that `--cmd` survives argparse — a subparser `dest="cmd"` silently overwrote it and the dispatch died looking up the user's whole crawllog invocation as a subcommand name. A hand-built namespace in a control cannot catch that, because it never runs argparse |
...
| `test_gsc.py` | the controls for the Search Console client: that the pure-Python RS256 signer verifies against its own public key and REFUSES a signature over other bytes, that a truncated key raises rather than returning a tiny modulus (a PKCS#1 parser pointed at PKCS#8 reads the version integer as the modulus), that string counts coerce and a missing one is `None` not 0, that a fully-lagged window is refused with the last complete day named, and that `sitemap-submit` cannot fire without `--yes` |
...
python3 $SEO/seostate.py deferred --due                               # withheld targets whose re-check condition is met
python3 $SEO/seostate.py defer --keyword "kw" --class catalogue --authority-count 6 --page1 a.com,b.com
...
# do answer engines cite us? (Google AI Overview works today via the SERP key)
python3 $SEO/geo.py engines                              # what can be asked RIGHT NOW
python3 $SEO/geo.py ask --query "..." --domain example.com
python3 $SEO/geo.py sweep --domain example.com --bank questions.txt --max 10
python3 $SEO/geo.py extractable --root ./public/seo/guides
# ⚠ `no_key` and `failing` are CANNOT ASK. Only an engine that ANSWERED can say
# `not_cited`, and a sweep with no reachable engine REFUSES rather than reporting that
# nobody cites you. This is the same rule as providers.py, and it is the whole instrument.
# ⚠ A SERP with no AI Overview is `no_answer_surface`, NOT a miss - counting it computes
# a citation rate against answers that never existed.
# ⚠ `share` is one vote per ANSWER, not per citation link, or a single answer linking a
# domain four times outweighs four answers linking it once (measured: share 4.2).
# ⚠ The engine's own domains are excluded and listed in `engine_furniture_excluded`.
# google.com appeared 21 times across 5 real answers and dominated the table.
# ⚠ SerpApi returns the AI Overview in TWO STAGES - the first response carries only a
# page_token, so reading `references` off it yields [] for every overview on earth. serp.py
# had exactly that bug: `present: true, references: []` always, i.e. "never cited", forever.

# did the change actually work? Register the question BEFORE you know the answer.
python3 $SEO/remeasure.py --root . record --id play-robots-block \
  --question "did the robots.txt block stop bingbot crawling /play?" \
  --cmd "crawllog.py scan --remote root@HOST --ssh-key K --glob '/var/log/caddy/access*.log*' --days 14 --bot bingbot --top 40" \
  --metric 'bots.0.top_silos./play' --missing-is-zero \
  --expect decrease --min-change 400 --after 21d
python3 $SEO/remeasure.py --root . due            # ready vs still waiting, with days left
python3 $SEO/remeasure.py --root . check --id play-robots-block
# ⚠ `--missing-is-zero` is OPT-IN and belongs only on SPARSE maps where absence really
# means zero (crawllog's top_silos omits a silo the bot never touched). As a default it
# turns every renamed key and every failed read into a zero.
# ⚠ `unmeasured` is not `no_change`. The hypothesis stays open and nothing was learned.
# ⚠ The command is stored WITH the baseline and re-run verbatim. A `--cmd` that differs is
# REFUSED. Quoting round-trips through shlex, so `serp.py 'cs 1.6 online'` survives.
...
# THE CRAWLER'S SIDE of the same account - what bingbot did, not what searchers did.
# First-party, no UA to verify, no log to parse. Pairs with crawllog.py: two
# instruments reading the same events, and a disagreement teaches you something.
python3 $SEO/bing.py crawlstats     # per day: pages fetched, code mix, pages in index
python3 $SEO/bing.py crawlissues    # per URL. An empty list is cross-checked, not trusted
python3 $SEO/bing.py feeds --verify # the sitemap AS BING HOLDS IT vs the live file
python3 $SEO/bing.py blocked        # account-level blocks - invisible to any crawl of the site
python3 $SEO/bing.py crawlsettings  # a self-imposed per-hour crawl-rate cap
# ⚠ crawlstats mixes DAILY counts with RUNNING TOTALS in one row and labels neither.
# Summing the wrong column gave Code2xx = 96,000 for a site with 7,408 pages crawled.
# The tool re-derives each column's kind per run and REFUSES to sum the cumulative
# ones; read latest_stock.<col>.latest, and .change_over_window for movement.
# ⚠ `all_other_codes` has no documented breakdown and ran at 0.88x pages crawled.
# It is reported under `caution` and is NOT an error rate.

# GETTING CRAWLED - two channels, separate quotas, and neither reaches Google.
python3 $SEO/bing.py quota                                    # read it immediately before a batch
python3 $SEO/bing.py submit --url https://example.com/a        # DRY RUN - shows, spends nothing
python3 $SEO/bing.py submit --from-file urls.txt --yes         # actually submits
python3 $SEO/indexnow.py ping --pending                        # the OTHER channel: keyless, also
                                                               # Yandex/Seznam/Naver/DuckDuckGo
# ⚠ `submit` is the only mutating call in bing.py. Off-site URLs are dropped before the
# call (Bing rejects the whole batch for one) and an over-quota batch is refused whole
# rather than truncated. Its receipt is for the REQUEST, not a crawl - verify later with
# `urlinfo`, with a control. ⚠ Check `remeasure.py due` first: submitting into a silo
# that is the subject of an open hypothesis is a second intervention inside someone
# else's experiment.
...
curl -s -m 900 -X POST localhost:8791/batch -H 'Content-Type: application/json' \
  -d '{"id":"my-run","queries":["kw one","kw two"],"depth":20}'   # compact verdicts, spooled under the id
curl -s 'localhost:8791/batch?id=my-run'                           # recover it if the POST's client timed out
